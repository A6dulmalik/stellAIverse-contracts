...
use soroban_sdk::{
    contracttype, format, Address, ConversionError, Env, String, Symbol, TryIntoVal, Val, Vec,
};
...
impl MarketplaceAtomicSupport {
    ...
    pub fn initialize(env: &Env) {
        ...
        let description = Some(String::from_str(
            env,
            "Atomic transaction support initialized",
        ));
    ...
    fn update_transaction_state(
        ...
        let mut tx_state = if let Some(state) = env
            .storage()
            .instance()
            .get::<_, AtomicTransactionState>(&tx_key)
        {
            state
        } else {
...
impl MarketplaceAtomicSupport {
    ...
    pub fn execute_atomic_transaction(
        ...
        let execution_order =
            stellai_lib::atomic::AtomicTransactionUtils::resolve_execution_order(env, steps);
        ...
        Self::create_atomic_audit_log(
            ...
            "transaction_started",
            true,
            Some("Starting atomic transaction execution"),
        );
            ...
            if let Some(step) = steps.iter().find(|s| s.step_id == step_id) {
                ...
                if !success {
                    ...
                    Self::rollback_transaction(
                        ...
                        steps,
                        executed_steps,
                        &format!("Step {} commit failed", step.step_id),
                    );
                    return false;
                }
        ...
        Self::create_atomic_audit_log(
            ...
            true,
            Some("All steps completed successfully"),
        );
    ...
    fn rollback_transaction(
        ...
        for step_id in executed_steps.iter().rev() {
            ...
            if let Some(step_state) = env
                .storage()
                .instance()
                .get::<_, AtomicStepState>(&step_key)
            {
        ...
        Self::create_atomic_audit_log(
            ...
            true,
            Some(&format!("Rollback complete: {}", reason)),
        );
...
impl AtomicTransactionSupport for MarketplaceAtomicSupport {
    fn prepare_step(
        ...
        // Validate that we can actually call this function (dry run preparation check)
        if let Err(_) =
            env.try_invoke_contract::<Val, ConversionError>(&step.contract, function, args.clone())
        {
            ...
            Self::create_atomic_audit_log(
                ...
                "prepare_failed",
                false,
                Some(error_msg.to_string().as_str()),
            );
            return false;
        }
        ...
        let log_msg = String::from_str(
            env,
            &format!("Function: {} prepared successfully", func_str),
        );
        Self::create_atomic_audit_log(
            env,
            transaction_id,
            ...
            .get::<_, AtomicStepState>(&step_key)
        {
            ...
            if state.executed {
                Self::create_atomic_audit_log(
                    ...
                    "commit_duplicate",
                    false,
                    Some("Step already executed"),
                );
                return true.into();
            }

            // Actually execute the step function using the stored contract address
            let execution_result = env.try_invoke_contract::<Val, ConversionError>(
                &state.contract,
                function,
                args.clone(),
            );
            ...
            let log_msg = String::from_str(
                env,
                &format!("Function: {} executed successfully", func_str),
            );
            Self::create_atomic_audit_log(
                env,
                transaction_id,
    ...
    fn rollback_step(
            ...
            .get::<_, AtomicStepState>(&step_key)
        {
            ...
            let rollback_result = env
                .try_invoke_contract::<Val, ConversionError>(
                    &rollback_contract,
                    rollback_function,
                    rollback_args.clone(),
                )
            ...
            Self::create_atomic_audit_log(
                ...
                rollback_result,
                Some(&format!(
                    "Rollback function: {} executed",
                    rollback_function.to_string()
                )),
            );
